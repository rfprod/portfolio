...
if [ 1 -gt $# ]; then
	...
	# prompt user for variables one by one: GITHUB_ACCESS_TOKEN
	...
	printf "${LIGHT_BLUE}  >> step ${YELLOW}6/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_HOST${LIGHT_BLUE} ${DEFAULT} \n"
	read -p "   > input value :" mailerHost

	# prompt user for variables one by one: MAILER_PORT
	printf "${LIGHT_BLUE}  >> step ${YELLOW}7/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_PORT${LIGHT_BLUE} ${DEFAULT} \n"
	read -p "   > input value :" mailerPort

	# prompt user for variables one by one: MAILER_EMAIL
	printf "${LIGHT_BLUE}  >> step ${YELLOW}8/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_EMAIL${LIGHT_BLUE} ${DEFAULT} \n"
	read -p "   > input value :" mailerEmail

	# prompt user for variables one by one: MAILER_CLIENT_ID
	printf "${LIGHT_BLUE}  >> step ${YELLOW}9/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_CLIENT_ID${LIGHT_BLUE} ${DEFAULT} \n"
	read -p "   > input value :" mailerClientId

	# prompt user for variables one by one: MAILER_CLIENT_SECRET
	printf "${LIGHT_BLUE}  >> step ${YELLOW}10/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_CLIENT_SECRET${LIGHT_BLUE} ${DEFAULT} \n"
	read -p "   > input value :" mailerClientSecret

	# prompt user for variables one by one: MAILER_REFRESH_TOKEN
	printf "${LIGHT_BLUE}  >> step ${YELLOW}11/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_REFRESH_TOKEN${LIGHT_BLUE} ${DEFAULT} \n"
	read -p "   > input value :" mailerRefreshToken

	# prompt user for variables one by one: MAILER_ACCESS_TOKEN
	printf "${LIGHT_BLUE}  >> step ${YELLOW}12/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_ACCESS_TOKEN${LIGHT_BLUE} ${DEFAULT} \n"
	read -p "   > input value :" mailerAccessToken

	# prompt user for variables one by one: MAILER_RECIPIENT_EMAIL
	printf "${LIGHT_BLUE}  >> step ${YELLOW}13/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_RECIPIENT_EMAIL${LIGHT_BLUE} ${DEFAULT} \n"
	read -p "   > input value :" mailerRecipientEmail

	# summary check
	printf "${LIGHT_BLUE}  >> step ${YELLOW}14/${totalSteps} ${LIGHT_BLUE}: You provided the following values:\n
	...
	# notify user
	printf "${LIGHT_BLUE}  >> step ${YELLOW}15/${totalSteps} ${LIGHT_BLUE}: Compare changes with existing ${YELLOW}./functions/.env${LIGHT_BLUE} file${DEFAULT} \n\n"
	cat ./functions/.env

	# prompt user whether to continue or not
	printf "\n${LIGHT_BLUE}  >> step ${YELLOW}16/${totalSteps} ${LIGHT_BLUE}: Continue and create/overwrite ${YELLOW}.env${LIGHT_BLUE} file${DEFAULT} \n"
	read -p "   > continue (y/n) :" userChoice
	case $userChoice in
		y|Y )
			# write file contents
